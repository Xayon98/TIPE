...
from process_annealing import pbar as pbar2
def dual_annealing(func, bounds, args=(), maxiter=1000,
    ...
    pbar2.write(f"Starting search at {time.strftime('%H:%M:%S', time.localtime())}")
    ...
    while not need_to_stop:
        for i in range(maxiter):
            ...
            # estimated_time = elapsed_time_seconds / iteration * (maxiter - iteration)
            # estimated_finish_time = time.strftime('%H:%M:%S', time.localtime(time.time() + estimated_time))
            # print(f"ETA: {estimated_finish_time}")
    ...
    pbar2.write(f"Finished search at {time.ctime()} in {time.strftime('%H:%M:%S', time.gmtime(time.time() - start_time))}")
